function         PL2est1(Data,parami,paramf,seed,stepi,flag_s,current_p,Datamat,lball,uball,PLk,minchi2)
numparam_HM=6;
...
lb=lball(subind);
ub=uball(subind);
options = optimset('Display','off','TolFun',1e-8,'TolX',1e-8,'MaxIter',5e3,'MaxFunEvals',7*5e3,'Jacobian','on');
...
try
    ...
    while Resnorm_temp_pre-Resnorm_temp>1e-7/2 && Resnorm_temp_pre>Resnorm_temp
